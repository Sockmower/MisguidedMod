...
import dev.sockmower.misguidedmod.repack.io.airlift.compress.Decompressor;
        ...
        implements Decompressor
{
    ...
    // Kantaja: why didn't this already exist?
    public static long getDecompressedSize(ByteBuffer input)
    {
        if (input.isDirect()) {
            long address = UnsafeUtil.getAddress(input) + input.position();
            return ZstdFrameDecompressor.getDecompressedSize(null, address, address + input.limit());
        }
        else if (input.hasArray()) {
            return getDecompressedSize(input.array(), input.arrayOffset(), input.limit() - input.arrayOffset());
        }
        else {
            throw new IllegalArgumentException("Unsupported input ByteBuffer implementation " + input.getClass().getName());
        }
