...
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.BitInputStream.peekBits;
...
class FiniteStateEntropy
{
    ...
    public static int decompress(FiniteStateEntropy.Table table, final Object inputBase, final long inputAddress, final long inputLimit, byte[] outputBuffer)
    {
        ...
        while (output <= outputLimit - 4) {
            ...
            UnsafeUtil.UNSAFE.putByte(outputBase, output + 1, symbols[state2]);
            numberOfBits = numbersOfBits[state2];
            state2 = (int) (newStates[state2] + peekBits(bitsConsumed, bits, numberOfBits));
            bitsConsumed += numberOfBits;

            UnsafeUtil.UNSAFE.putByte(outputBase, output + 2, symbols[state1]);
            numberOfBits = numbersOfBits[state1];
            state1 = (int) (newStates[state1] + peekBits(bitsConsumed, bits, numberOfBits));
            bitsConsumed += numberOfBits;

            UnsafeUtil.UNSAFE.putByte(outputBase, output + 3, symbols[state2]);
            numberOfBits = numbersOfBits[state2];
            state2 = (int) (newStates[state2] + peekBits(bitsConsumed, bits, numberOfBits));
            bitsConsumed += numberOfBits;
    ...
    {
        Util.checkArgument(outputSize >= SIZE_OF_LONG, "Output buffer too small");
        ...
        while (remaining > 1) {
            if (previousIs0) {
                ...
                while (symbol >= start + 24) {
                    ...
                    Util.checkArgument(output + SIZE_OF_SHORT <= outputLimit, "Output buffer too small");
