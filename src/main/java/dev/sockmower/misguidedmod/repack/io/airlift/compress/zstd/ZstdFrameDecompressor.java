...
import dev.sockmower.misguidedmod.repack.io.airlift.compress.MalformedInputException;

import java.util.Arrays;

import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.UnsafeUtil.UNSAFE;
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Util.fail;
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Util.mask;
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Util.verify;
...
class ZstdFrameDecompressor
{
    ...
    private final byte[] literals = new byte[Constants.MAX_BLOCK_SIZE + Constants.SIZE_OF_LONG]; // extra space to allow for long-at-a-time copy
    ...
    private final FiniteStateEntropy.Table literalsLengthTable = new FiniteStateEntropy.Table(Constants.LITERAL_LENGTH_TABLE_LOG);
    private final FiniteStateEntropy.Table offsetCodesTable = new FiniteStateEntropy.Table(Constants.OFFSET_TABLE_LOG);
    private final FiniteStateEntropy.Table matchLengthTable = new FiniteStateEntropy.Table(Constants.MATCH_LENGTH_TABLE_LOG);
            ...
            final long outputLimit)
    {
        ...
        while (input < inputLimit) {
            ...
            boolean lastBlock;
            do {
                verify(input + Constants.SIZE_OF_BLOCK_HEADER <= inputLimit, input, "Not enough input bytes");
    ...
    private int decodeCompressedBlock(Object inputBase, final long inputAddress, int blockSize, Object outputBase, long outputAddress, long outputLimit, int windowSize, long outputAbsoluteBaseAddress)
    {
        ...
        verify(blockSize <= Constants.MAX_BLOCK_SIZE, input, "Expected match length table to be present");
        verify(blockSize >= Constants.MIN_BLOCK_SIZE, input, "Compressed block size too small");
            ...
            long outputAbsoluteBaseAddress)
    {
        ...
        verify(size >= Constants.MIN_SEQUENCES_SIZE, input, "Not enough input bytes");

        // decode header
        int sequenceCount = UNSAFE.getByte(inputBase, input++) & 0xFF;
        if (sequenceCount != 0) {
            if (sequenceCount == 255) {
                verify(input + Constants.SIZE_OF_SHORT <= inputLimit, input, "Not enough input bytes");
                sequenceCount = (UNSAFE.getShort(inputBase, input) & 0xFFFF) + Constants.LONG_NUMBER_OF_SEQUENCES;
            ...
            verify(input + Constants.SIZE_OF_INT <= inputLimit, input, "Not enough input bytes");
    ...
    private long computeMatchLengthTable(int matchLengthType, Object inputBase, long input, long inputLimit)
    {
        switch (matchLengthType) {
            ...
            case Constants.SEQUENCE_ENCODING_COMPRESSED:
                input += fse.readFseTable(matchLengthTable, inputBase, input, inputLimit, Constants.MAX_MATCH_LENGTH_SYMBOL, Constants.MATCH_LENGTH_TABLE_LOG);
                currentMatchLengthTable = matchLengthTable;
                break;
            default:
    ...
    private long computeOffsetsTable(int offsetCodesType, Object inputBase, long input, long inputLimit)
    {
        switch (offsetCodesType) {
            ...
            case Constants.SEQUENCE_ENCODING_REPEAT:
                ...
                break;
            case Constants.SEQUENCE_ENCODING_COMPRESSED:
                input += fse.readFseTable(offsetCodesTable, inputBase, input, inputLimit, Constants.DEFAULT_MAX_OFFSET_CODE_SYMBOL, Constants.OFFSET_TABLE_LOG);
                currentOffsetCodesTable = offsetCodesTable;
                break;
            default:
    ...
    private long computeLiteralsTable(int literalsLengthType, Object inputBase, long input, long inputLimit)
    {
        switch (literalsLengthType) {
            ...
            case Constants.SEQUENCE_ENCODING_REPEAT:
                ...
                break;
            case Constants.SEQUENCE_ENCODING_COMPRESSED:
                input += fse.readFseTable(literalsLengthTable, inputBase, input, inputLimit, Constants.MAX_LITERALS_LENGTH_SYMBOL, Constants.LITERAL_LENGTH_TABLE_LOG);
                currentLiteralsLengthTable = literalsLengthTable;
                break;
            default:
    ...
    private int decodeCompressedLiterals(Object inputBase, final long inputAddress, int blockSize, int literalsBlockType)
    {
        ...
        verify(uncompressedSize <= Constants.MAX_BLOCK_SIZE, input, "Block exceeds maximum size");
    ...
    private int decodeRleLiterals(Object inputBase, final long inputAddress, int blockSize)
    {
        ...
        switch (type) {
            ...
            case 3:
                ...
                verify(blockSize >= Constants.SIZE_OF_INT, input, "Not enough input bytes");
        ...
        verify(outputSize <= Constants.MAX_BLOCK_SIZE, input, "Output exceeds maximum block size");
