...
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.BitInputStream.isEndOfStream;
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.BitInputStream.peekBitsFast;
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Constants.SIZE_OF_INT;
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Constants.SIZE_OF_SHORT;
...
class Huffman
{
    ...
    public int readTable(final Object inputBase, final long inputAddress, final int size)
    {
        ...
        if (inputSize >= 128) {
            ...
            for (int i = 0; i < outputSize; i += 2) {
                int value = UnsafeUtil.UNSAFE.getByte(inputBase, input + i / 2) & 0xFF;
    ...
    public void decode4Streams(final Object inputBase, final long inputAddress, final long inputLimit, final Object outputBase, final long outputAddress, final long outputLimit)
    {
        ...
        long start2 = start1 + (UnsafeUtil.UNSAFE.getShort(inputBase, inputAddress) & 0xFFFF);
        long start3 = start2 + (UnsafeUtil.UNSAFE.getShort(inputBase, inputAddress + 2) & 0xFFFF);
        long start4 = start3 + (UnsafeUtil.UNSAFE.getShort(inputBase, inputAddress + 4) & 0xFFFF);
    ...
    private void decodeTail(final Object inputBase, final long startAddress, long currentAddress, int bitsConsumed, long bits, final Object outputBase, long outputAddress, final long outputLimit)
    {
        ...
        Util.verify(isEndOfStream(startAddress, currentAddress, bitsConsumed), startAddress, "Bit stream is not fully consumed");
    }
