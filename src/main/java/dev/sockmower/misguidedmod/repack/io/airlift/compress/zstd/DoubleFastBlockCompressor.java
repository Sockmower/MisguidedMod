...
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Constants.SIZE_OF_INT;
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Constants.SIZE_OF_LONG;
        ...
        implements BlockCompressor
{
    ...
    public int compressBlock(Object inputBase, final long inputAddress, int inputSize, SequenceStore output, BlockCompressionState state, RepeatedOffsets offsets, CompressionParameters parameters)
    {
        ...
        while (input < inputLimit) {   // < instead of <=, because repcode check at (input+1)
            ...
            if (offset1 > 0 && UnsafeUtil.UNSAFE.getInt(inputBase, input + 1 - offset1) == UnsafeUtil.UNSAFE.getInt(inputBase, input + 1)) {
                ...
                if (longMatchAddress > windowBaseAddress && UnsafeUtil.UNSAFE.getLong(inputBase, longMatchAddress) == UnsafeUtil.UNSAFE.getLong(inputBase, input)) {
                    ...
                    while (input > anchor && longMatchAddress > windowBaseAddress && UnsafeUtil.UNSAFE.getByte(inputBase, input - 1) == UnsafeUtil.UNSAFE.getByte(inputBase, longMatchAddress - 1)) {
                        input--;
                        longMatchAddress--;
                        matchLength++;
                    }
                    ...
                    if (shortMatchAddress > windowBaseAddress && UnsafeUtil.UNSAFE.getInt(inputBase, shortMatchAddress) == UnsafeUtil.UNSAFE.getInt(inputBase, input)) {
                        int nextOffsetHash = hash8(UnsafeUtil.UNSAFE.getLong(inputBase, input + 1), longHashBits);
                        long nextOffsetMatchAddress = baseAddress + longHashTable[nextOffsetHash];
                        longHashTable[nextOffsetHash] = current + 1;

                        // check prefix long +1 match
                        if (nextOffsetMatchAddress > windowBaseAddress && UnsafeUtil.UNSAFE.getLong(inputBase, nextOffsetMatchAddress) == UnsafeUtil.UNSAFE.getLong(inputBase, input + 1)) {
                            ...
                            while (input > anchor && nextOffsetMatchAddress > windowBaseAddress && UnsafeUtil.UNSAFE.getByte(inputBase, input - 1) == UnsafeUtil.UNSAFE.getByte(inputBase, nextOffsetMatchAddress - 1)) {
                                input--;
                            ...
                            while (input > anchor && shortMatchAddress > windowBaseAddress && UnsafeUtil.UNSAFE.getByte(inputBase, input - 1) == UnsafeUtil.UNSAFE.getByte(inputBase, shortMatchAddress - 1)) {
                                input--;
                                shortMatchAddress--;
                                matchLength++;
                            }
            ...
            if (input <= inputLimit) {
                // Fill Table
                longHashTable[hash8(UnsafeUtil.UNSAFE.getLong(inputBase, baseAddress + current + 2), longHashBits)] = current + 2;
                shortHashTable[hash(inputBase, baseAddress + current + 2, shortHashBits, matchSearchLength)] = current + 2;

                longHashTable[hash8(UnsafeUtil.UNSAFE.getLong(inputBase, input - 2), longHashBits)] = (int) (input - 2 - baseAddress);
                shortHashTable[hash(inputBase, input - 2, shortHashBits, matchSearchLength)] = (int) (input - 2 - baseAddress);

                while (input <= inputLimit && offset2 > 0 && UnsafeUtil.UNSAFE.getInt(inputBase, input) == UnsafeUtil.UNSAFE.getInt(inputBase, input - offset2)) {
