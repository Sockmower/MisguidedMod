...
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Util.checkArgument;
import static dev.sockmower.misguidedmod.repack.io.airlift.compress.zstd.Util.put24BitLittleEndian;
...
class ZstdFrameCompressor
{
    ...
    {
        checkArgument(outputLimit - outputAddress >= Constants.SIZE_OF_INT, "Output buffer too small");

        UNSAFE.putInt(outputBase, outputAddress, Constants.MAGIC_NUMBER);
    ...
    private static int compressFrame(Object inputBase, long inputAddress, long inputLimit, Object outputBase, long outputAddress, long outputLimit, CompressionParameters parameters)
    {
        ...
        do {
            checkArgument(outputSize >= Constants.SIZE_OF_BLOCK_HEADER + Constants.MIN_BLOCK_SIZE, "Output buffer too small");
            ...
            if (remaining > 0) {
                compressedSize = compressBlock(inputBase, input, blockSize, outputBase, output + Constants.SIZE_OF_BLOCK_HEADER, outputSize - Constants.SIZE_OF_BLOCK_HEADER, context, parameters);
            }

            if (compressedSize == 0) { // block is not compressible
                checkArgument(blockSize + Constants.SIZE_OF_BLOCK_HEADER <= outputSize, "Output size too small");

                int blockHeader = lastBlockFlag | (Constants.RAW_BLOCK << 1) | (blockSize << 3);
                put24BitLittleEndian(outputBase, output, blockHeader);
                UNSAFE.copyMemory(inputBase, input, outputBase, output + Constants.SIZE_OF_BLOCK_HEADER, blockSize);
            ...
            int literalsSize)
    {
        ...
        switch (headerSize) {
            ...
            case 5: { // 2 - 2 - 18 - 18
                ...
                UNSAFE.putInt(outputBase, outputAddress, header);
                UNSAFE.putByte(outputBase, outputAddress + Constants.SIZE_OF_INT, (byte) (totalSize >>> 10));
                break;
            }
    ...
    private static int rleLiterals(Object outputBase, long outputAddress, int outputSize, Object inputBase, long inputAddress, int inputSize)
    {
        ...
        switch (headerSize) {
            case 1: // 2 - 1 - 5
                UNSAFE.putByte(outputBase, outputAddress, (byte) (Constants.RLE_LITERALS_BLOCK | (inputSize << 3)));
                break;
            case 2: // 2 - 2 - 12
                UNSAFE.putShort(outputBase, outputAddress, (short) (Constants.RLE_LITERALS_BLOCK | (1 << 2) | (inputSize << 4)));
                break;
            case 3: // 2 - 2 - 20
                UNSAFE.putInt(outputBase, outputAddress, Constants.RLE_LITERALS_BLOCK | 3 << 2 | inputSize << 4);
                break;
    ...
    private static int rawLiterals(Object outputBase, long outputAddress, int outputSize, Object inputBase, long inputAddress, int inputSize)
    {
        ...
        switch (headerSize) {
            case 1:
                UNSAFE.putByte(outputBase, outputAddress, (byte) (Constants.RAW_LITERALS_BLOCK | (inputSize << 3)));
                break;
            case 2:
                UNSAFE.putShort(outputBase, outputAddress, (short) (Constants.RAW_LITERALS_BLOCK | (1 << 2) | (inputSize << 4)));
                break;
            case 3:
                put24BitLittleEndian(outputBase, outputAddress, Constants.RAW_LITERALS_BLOCK | (3 << 2) | (inputSize << 4));
                break;
            default:
                throw new AssertionError();
        }
