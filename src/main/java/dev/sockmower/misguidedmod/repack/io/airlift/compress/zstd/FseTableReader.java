...
{
    private final short[] nextSymbol = new short[FiniteStateEntropy.MAX_SYMBOL + 1];
    private final short[] normalizedCounters = new short[FiniteStateEntropy.MAX_SYMBOL + 1];
    ...
    public int readFseTable(FiniteStateEntropy.Table table, Object inputBase, long inputAddress, long inputLimit, int maxSymbol, int maxTableLog)
    {
        ...
        while (remaining > 1 && symbolNumber <= maxSymbol) {
            ...
            bitStream = UnsafeUtil.UNSAFE.getInt(inputBase, input) >>> (bitCount & 31);
        }
