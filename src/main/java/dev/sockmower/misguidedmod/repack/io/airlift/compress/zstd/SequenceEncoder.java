...
class SequenceEncoder
{
    ...
    private static final FseCompressionTable DEFAULT_LITERAL_LENGTHS_TABLE = FseCompressionTable.newInstance(DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS, Constants.MAX_LITERALS_LENGTH_SYMBOL, DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS_LOG);
    private static final FseCompressionTable DEFAULT_MATCH_LENGTHS_TABLE = FseCompressionTable.newInstance(DEFAULT_MATCH_LENGTH_NORMALIZED_COUNTS, Constants.MAX_MATCH_LENGTH_SYMBOL, DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS_LOG);
    private static final FseCompressionTable DEFAULT_OFFSETS_TABLE = FseCompressionTable.newInstance(DEFAULT_OFFSET_NORMALIZED_COUNTS, Constants.DEFAULT_MAX_OFFSET_CODE_SYMBOL, DEFAULT_OFFSET_NORMALIZED_COUNTS_LOG);
    ...
    public static int compressSequences(Object outputBase, final long outputAddress, int outputSize, SequenceStore sequences, CompressionParameters.Strategy strategy, SequenceEncodingContext workspace)
    {
        ...
        if (sequenceCount < 0x7F) {
            UnsafeUtil.UNSAFE.putByte(outputBase, output, (byte) sequenceCount);
            output++;
        }
        else if (sequenceCount < Constants.LONG_NUMBER_OF_SEQUENCES) {
            UnsafeUtil.UNSAFE.putByte(outputBase, output, (byte) (sequenceCount >>> 8 | 0x80));
            UnsafeUtil.UNSAFE.putByte(outputBase, output + 1, (byte) sequenceCount);
            output += Constants.SIZE_OF_SHORT;
        }
        else {
            UnsafeUtil.UNSAFE.putByte(outputBase, output, (byte) 0xFF);
            output++;
            UnsafeUtil.UNSAFE.putShort(outputBase, output, (short) (sequenceCount - Constants.LONG_NUMBER_OF_SEQUENCES));
            output += Constants.SIZE_OF_SHORT;
        }
        ...
        boolean defaultAllowed = maxSymbol < Constants.DEFAULT_MAX_OFFSET_CODE_SYMBOL;
